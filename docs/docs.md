...
See [Creating the tracker instance](creating_the_tracker_instance.md) for more details.
...
See [Sending events](sending_events.md) for more details.
...
See [Setting login status](setting_login_status.md) for more details.
...
| ---------- | --------------- | ----------- | ------- |
| autoSearch  | Yes | After performing a search using a GroupBy search API, this is used for sending details of the search to GroupBy's beacon API. The details are sent from the web browser using this event instead of being retrieved internally by GroupBy so that client tracking works correctly and aligns with the rest of the event types which must be sent from the client. | [autoSearch](autoSearch.md)
| viewProduct  | Yes | For sending details of which product (or SKU within a product) the shopper is viewing details of. | [viewProduct](viewProduct.md)
| addToCart | Yes | For sending details of which products (or SKUs within products) the shopper is adding to their cart. | [autoSearch](addToCart.md)
| removeFromCart | No | For sending details of which products (or SKUs within products) the shopper is removing from their cart. | [removeFromCart](removeFromCart.md)
| order | Yes | For sending details of which products (or SKUs within products) the shopper is ordering. | [order](order.md)
| recImpression | No | For sending details of which products (or SKUs within products) the shopper is viewing on a page where you're rendering recommendations from a GroupBy recommendation API. | [recImpression](recImpression.md)
